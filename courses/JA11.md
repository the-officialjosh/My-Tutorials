...
Say we have a client and we want to get some data from the server. We can do this by sending a request to the server. The server will then process the request and send back a response. This is called the `request-response cycle`. <br>
![Alt text](image.png)